<template>
    <a-page-header class="b-header" :isScroll="isScroll">
        ...
        <template #tags>
            <a-tag v-for="tag in tags" :color="tag.color" :key="tag.tagName">{{ tag.tagName }}</a-tag>
        </template>
        <template #extra>
            <a-button v-for="btn in menu" :type="btn.type" @click="handle" :key="btn.text">{{ btn.text }}</a-button>
        </template>
    </a-page-header>
</template>
